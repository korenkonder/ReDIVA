...
typedef struct NPD_HEADER {
    ...
    uint8_t digest[0x10];
    uint8_t title_hash[0x10];
    uint8_t dev_hash[0x10];
    int64_t unk1;
    int64_t unk2;
} NPD_HEADER;

typedef struct EDAT_HEADER {
    int32_t flags;
    int32_t block_size;
    int64_t file_size;
} EDAT_HEADER;
...
extern void DecryptEDAT(stream* input, stream* output, int mode, char* rap_file_name, uint8_t* custom_klic);
