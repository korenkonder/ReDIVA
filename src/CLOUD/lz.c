...
int decompress(unsigned char* out, unsigned char* in, unsigned int size)
{
    ...
    while (1) {
        ...
        if (!decode_bit(&range, &code, 0, &in, tmp_sect1)) { // Raw char.
            ...
            if (offset > 0) offset--;
            if (start == end) return (int)(start - out);
            ...
            *start++ = index;
            ...
            if ((index >= 0) || (bit_flag != 0)) {
                ...
                if (data_length == 0xFF) return (int)(start - out);  // End of stream.
            }
            else
            ...
            do {
                diff = (shift << 4) - b_size;
                bit_flag = decode_bit(&range, &code, &shift, &in, tmp_sect2 + ((size_t)shift << 3));
            } while (diff < 0);
