...
static const object_info object_info_null = { (uint16_t)-1, (uint16_t)-1 };

static bone_database_struct** bone_database_ptr = (bone_database_struct**)0x00000000140FBC1C0;
...
static bone_data* bone_data_init(bone_data* bone) {
    bone->type = (bone_database_bone_type)0;
    bone->has_parent = 0;
    ...
    for (int32_t i = 0; i < 2; i++)
        for (int32_t j = 0; j < 3; j++)
            bone->rot_mat_prev[i][j] = mat4_null;

    bone->pole_target_mat = 0;
    bone->parent_mat = 0;
    bone->node = 0;
    bone->arm_length = 0.0f;
    bone->eyes_xrot_adjust_neg = 1.0f;
    bone->eyes_xrot_adjust_pos = 1.0f;
    return bone;
}

static void bone_database_bones_calculate_count(bone_database_bone* bones, size_t* object_bone_count,
    ...
    size_t _chain_pos = 0;
    for (bone_database_bone* i = bones; i->type != BONE_DATABASE_BONE_END; i++)
        switch (i->type) {
...
static bone_database_skeleton* bone_database_get_skeleton_by_type(bone_database_skeleton_type type) {
    if (type == BONE_DATABASE_SKELETON_NONE || !*bone_database_ptr
...
static bone_database_bone* bone_database_get_bones_by_type(bone_database_skeleton_type type) {
    bone_database_skeleton* skel = bone_database_get_skeleton_by_type(type);
    if (!skel)
        return 0;
...
static uint16_t* bone_database_get_skeleton_parent_indices_by_type(bone_database_skeleton_type type) {
    bone_database_skeleton* skel = bone_database_get_skeleton_by_type(type);
    if (!skel)
        return 0;
...
static vec3* bone_database_get_skeleton_positions_by_type(bone_database_skeleton_type type) {
    bone_database_skeleton* skel = bone_database_get_skeleton_by_type(type);
    if (!skel)
        return 0;
...
static float_t bone_database_get_skeleton_heel_height(bone_database_skeleton_type a1) {
    bone_database_skeleton* skel = bone_database_get_skeleton_by_type(a1);
    if (skel)
...
static void rob_chara_bone_data_set_mats(rob_chara_bone_data* rob_bone_data,
    ...
    for (bone_database_bone* i = a2; i->type != BONE_DATABASE_BONE_END; i++)
        switch (i->type) {
        case BONE_DATABASE_BONE_ROTATION:
        case BONE_DATABASE_BONE_TYPE_1:
        case BONE_DATABASE_BONE_POSITION:
...
static void bone_data_parent_load_bone_database(bone_data_parent* bone,
    bone_database_bone* bones, vec3* common_translation, vec3* translation) {
    rob_chara_bone_data* rob_bone_data = bone->rob_bone_data;
    size_t chain_pos = 0;
    size_t total_bone_count = 0;
    size_t ik_bone_count = 0;

    bone_database_bone* v15 = bones;
    bone_data* v16 = bone->bones.begin;
    for (size_t i = 0; v15->type != BONE_DATABASE_BONE_END; i++, v15++, v16++) {
        v16->motion_bone_index = (motion_bone_index)i;
        v16->type = (bone_database_bone_type)v15->type;
...
static void bone_data_parent_data_init(bone_data_parent* bone,
    ...
    bone_database_bone* common_bone_data = bone_database_get_bones_by_type(rob_bone_data->base_skeleton_type);
    vec3* common_translation = bone_database_get_skeleton_positions_by_type(rob_bone_data->base_skeleton_type);
    vec3* translation = bone_database_get_skeleton_positions_by_type(rob_bone_data->skeleton_type);
...
void bone_data_mult_ik(bone_data* a1, int32_t skeleton_select) {
    ...
    float_t v25;
    if (v8 > 0.000001f) {
        float_t arm_length = a1->arm_length;
        if (arm_length > 0.0001f) {
            float_t v27 = (v21 + v20) * arm_length;
            if (v10 > v27) {
                v10 = v27;
                v8 = v27 * v27;
            }
...
static bool sub_1404136B0(motion_blend_mot* a1) {
    ...
    if (v1)
        return v1->__vftable->Field30(v1);
    else
        return false;
}
...
static void sub_140410A40(motion_blend_mot* a1, vector_old_bone_data* a2, vector_old_bone_data* a3) {
    ...
    for (bone_data* i = a2->begin; i != a2->end; i++) {
        ...
        v1->__vftable->Blend(v1, &a2->begin[i->motion_bone_index], v8);
    }
}
...
static void sub_140410B70(motion_blend_mot* a1, vector_old_bone_data* a2) {
    ...
    v1->__vftable->Field20(v1, &a1->bone_data.bones, a2);
    for (bone_data* i = a1->bone_data.bones.begin; i != a1->bone_data.bones.end; i++) {
        if (!sub_140410250(&a1->field_0.field_8, i->motion_bone_index))
            continue;
        ...
        v1->__vftable->Blend(a1->blend, &a1->bone_data.bones.begin[i->motion_bone_index], v6);
    }
}

static MotionBlendType sub_1404125C0(motion_blend_mot* a1) {
    MotionBlend* v1 = a1->blend;
    if (v1 == &a1->cross.base)
...
static void sub_140410CB0(mot_blend* a1, vector_old_bone_data* a2) {
    MotionBlend* v1 = &a1->blend.base;
    if (!v1->rot_y)
        return;

    for (bone_data* i = a2->begin; i != a2->end; i++)
        if (sub_140410250(&a1->field_0.field_8, i->motion_bone_index))
            v1->__vftable->Blend(&a1->blend.base, &a2->begin[i->motion_bone_index], 0);
}
...
static void rob_chara_bone_data_init_skeleton(rob_chara_bone_data* rob_bone_data,
    ...
    bone_database_bone* bones = bone_database_get_bones_by_type(base_skeleton_type);
    ...
    uint16_t* parent_indices = bone_database_get_skeleton_parent_indices_by_type(base_skeleton_type);
...
static void mot_key_data_get_key_set_count_by_bone_database_bones(mot_key_data* a1, bone_database_bone* bones) {
    size_t object_bone_count;
    size_t motion_bone_count;
    size_t total_bone_count;
    size_t ik_bone_count;
    size_t chain_pos;
    bone_database_bones_calculate_count(bones, &object_bone_count,
...
static void mot_key_data_reserve_key_sets_by_skeleton_type(mot_key_data* a1,
    bone_database_skeleton_type type) {
    mot_key_data_get_key_set_count_by_bone_database_bones(a1, bone_database_get_bones_by_type(type));
    mot_key_data_reserve_key_sets(a1);
    a1->skeleton_type = type;
}
...
static void rob_sub_action_reset(RobSubAction* sub_act) {
...
static void rob_chara_motion_reset(rob_chara_motion* a1) {
    a1->motion_id = -1;
    a1->prev_motion_id = -1;
    ...
    a1->field_140 = 0;
    a1->field_144 = 0;
    a1->field_150.face.base.__vftable->Reset(&a1->field_150.face.base);
    a1->field_150.hand_l.base.__vftable->Reset(&a1->field_150.hand_l.base);
    a1->field_150.hand_r.base.__vftable->Reset(&a1->field_150.hand_r.base);
    a1->field_150.mouth.base.__vftable->Reset(&a1->field_150.mouth.base);
    a1->field_150.eye.base.__vftable->Reset(&a1->field_150.eye.base);
    a1->field_150.eyelid.base.__vftable->Reset(&a1->field_150.eyelid.base);
    a1->field_150.head_object = object_info_null;
    a1->field_150.hand_l_object = object_info_null;
    a1->field_150.hand_r_object = object_info_null;
    a1->field_150.face_object = object_info_null;
    a1->field_150.field_1C0 = 0;
    a1->field_150.time = 0.0f;
    a1->hand_l.base.__vftable->Reset(&a1->hand_l.base);
    a1->hand_r.base.__vftable->Reset(&a1->hand_r.base);
    a1->hand_l_object = object_info_null;
    a1->hand_r_object = object_info_null;
    a1->field_3B0.face.base.__vftable->Reset(&a1->field_3B0.face.base);
    a1->field_3B0.hand_l.base.__vftable->Reset(&a1->field_3B0.hand_l.base);
    a1->field_3B0.hand_r.base.__vftable->Reset(&a1->field_3B0.hand_r.base);
    a1->field_3B0.mouth.base.__vftable->Reset(&a1->field_3B0.mouth.base);
    a1->field_3B0.eye.base.__vftable->Reset(&a1->field_3B0.eye.base);
    a1->field_3B0.eyelid.base.__vftable->Reset(&a1->field_3B0.eyelid.base);
    a1->field_3B0.head_object = object_info_null;
    a1->field_3B0.hand_l_object = object_info_null;
    a1->field_3B0.hand_r_object = object_info_null;
    a1->field_3B0.face_object = object_info_null;
    a1->field_3B0.field_1C0 = 0;
    a1->field_3B0.time = 0.0f;

    rob_chara_data_adjust* parts_adjust = a1->parts_adjust;
    rob_chara_data_adjust* parts_adjust_prev = a1->parts_adjust_prev;
    ...
    rob_chara_data_adjust* v7 = &a1->adjust_global;
    rob_chara_data_arm_adjust* v8 = a1->arm_adjust;
    ...
    for (int32_t i = 0; i < 2; i++) {
        ...
        hand_adjust_prev->type = ROB_CHARA_DATA_HAND_ADJUST_NONE;
        hand_adjust_prev->current_scale = 1.0f;
        hand_adjust_prev->scale = 1.0f;
        hand_adjust_prev->duration = 0.0f;
        hand_adjust_prev->current_time = 0.0f;
        hand_adjust_prev->rotation_blend = 1.0f;
        hand_adjust_prev->scale_blend = 1.0f;
        hand_adjust_prev->enable_scale = false;
...
static void sub_140539750(struc_223* a1) {
    ...
    sub_14053A660(&a1->field_0.field_20);
    sub_14053A660(&a1->field_0.field_10);
    sub_140537110(a1);
    sub_1405335C0(a1);
    a1->field_7A0 = 0;
    a1->motion_set_id = -1;
...
static void rob_chara_data_reset(rob_chara_data* a1) {
    ...
    rob_sub_action_reset(&a1->rob_sub_action);
    rob_chara_motion_reset(&a1->motion);
...
static void motion_blend_mot_reset(motion_blend_mot* a1) {
    ...
    a1->mot_key_data.skeleton_type = BONE_DATABASE_SKELETON_NONE;
    ...
    a1->mot_play_data.frame_data.step = 1.0f;
    a1->mot_play_data.frame_data.step_prev = 1.0f;
    a1->mot_play_data.frame_data.frame_count = 0.0f;
    a1->mot_play_data.frame_data.last_frame = -1.0f;
    ...
    a1->cross.base.__vftable->Reset(&a1->cross.base);
    a1->freeze.base.__vftable->Reset(&a1->freeze.base);
    a1->combine.base.base.__vftable->Reset(&a1->combine.base.base);
    a1->blend = 0;
}
...
static void mot_blend_reset(mot_blend* a1) {
    ...
    a1->mot_play_data.field_40 = 0;
    a1->mot_play_data.field_48 = 0;
    a1->blend.base.__vftable->Reset(&a1->blend.base);
    a1->field_0.bone_check_func = 0;
    sub_1404119A0(&a1->field_0.field_8);
    a1->field_0.motion_bone_count = 0;
    a1->field_30 = 0;
...
static void rob_chara_bone_data_reset(rob_chara_bone_data* rob_bone_data) {
    ...
    rob_bone_data->disable_eye_motion = 0;
...
static skeleton_rotation_offset* skeleton_rotation_offset_array_get(bone_database_skeleton_type type) {
    skeleton_rotation_offset** skeleton_rotation_offset_array_ptr
        = (skeleton_rotation_offset**)0x0000000140A01560;
    if (type >= BONE_DATABASE_SKELETON_COMMON && type <= BONE_DATABASE_SKELETON_TETO)
...
static void sub_1401EB1D0(bone_data* a1, int32_t a2) {
    ...
    if (a1->type >= BONE_DATABASE_BONE_TYPE_1 && a1->type <= BONE_DATABASE_BONE_POSITION_ROTATION)
        a1->trans_prev[a2] = a1->trans;

    switch (a1->type) {
    case BONE_DATABASE_BONE_ARM_IK_ROTATION:
    case BONE_DATABASE_BONE_LEGS_IK_ROTATION:
        a1->rot_mat_prev[2][a2] = a1->rot_mat[2];
    case BONE_DATABASE_BONE_HEAD_IK_ROTATION:
        a1->rot_mat_prev[1][a2] = a1->rot_mat[1];
    case BONE_DATABASE_BONE_ROTATION:
    case BONE_DATABASE_BONE_TYPE_1:
    case BONE_DATABASE_BONE_POSITION:
    case BONE_DATABASE_BONE_POSITION_ROTATION:
    default:
        a1->rot_mat_prev[0][a2] = a1->rot_mat[0];
        break;
    }
}
...
static void sub_14041AD90(rob_chara_bone_data* a1) {
    ...
    else if (a1->eye.mot_key_data.key_sets_ready && a1->eye.mot_key_data.mot_data && !a1->eye.field_30
        || a1->disable_eye_motion)
        sub_140413EA0(&a1->face.field_0, sub_14041B440);
}

static void sub_14041AD50(rob_chara_bone_data* a1) {
    if (a1->eye.mot_key_data.key_sets_ready && a1->eye.mot_key_data.mot_data && !a1->eye.field_30
        || a1->disable_eye_motion)
...
static void sub_14041ABA0(rob_chara_bone_data* a1) {
    ...
    while (v3 != v1) {
        if (a1->face.mot_key_data.key_sets_ready
            ...
            if (!a1->disable_eye_motion || a1->eye.mot_key_data.key_sets_ready
...
static void sub_140415B30(mot_blend* a1) {
    ...
    v3.rot_y = 0.0f;
    v3.prev_rot_y = 0.0f;
    a1->blend.base.__vftable->Step(&a1->blend.base, &v3);
}
...
static void sub_140412E10(motion_blend_mot* a1, int32_t a2) {
    for (bone_data* i = a1->bone_data.bones.begin; i != a1->bone_data.bones.end; i++) {
        ...
        sub_1401EB1D0(i, a2);
        if (i->type == BONE_DATABASE_BONE_POSITION_ROTATION && a1->field_4F8.field_0 & 2)
            vec3_add(i->trans_prev[a2], a1->field_4F8.field_90, i->trans_prev[a2]);
    }
}
...
static void sub_1404148C0(motion_blend_mot* a1, float_t a2) {
    MotionBlend* v2 = a1->blend;
    if (v2)
        sub_1403FBF10(v2, a2);
}

static void sub_1404147F0(motion_blend_mot* a1, MotionBlendType type, float_t blend) {
...
static int32_t sub_1401F0E70(bone_database_skeleton_type type, const char* name) {
    bone_database_struct* bone_database
        = *(bone_database_struct**)0x0000000140FBC1C0;
    int32_t(*sub_1401F0EA0)(const char* a1, const char* a2)
        = (int32_t(*)(const char*, const char*))0x00000001401F0EA0;

    if (!bone_database || type == BONE_DATABASE_SKELETON_NONE)
        return -1;
    else
...
static void motion_blend_mot_load_file(motion_blend_mot* a1,
    int32_t motion_id, MotionBlendType a3, float_t blend) {
...
void rob_chara_bone_data_motion_load(rob_chara_bone_data* rob_bone_data,
    int32_t motion_id, MotionBlendType motion_blend_type) {
    void(*rob_chara_bone_data_motion_blend_mot_list_init)(rob_chara_bone_data * rob_bone_data)
        = (void(*)(rob_chara_bone_data*))0x000000014041A9E0;
    void(*rob_chara_bone_data_motion_blend_mot_list_free)(rob_chara_bone_data * rob_bone_data, size_t a2)
        = (void(*)(rob_chara_bone_data*, size_t))0x0000000140418E80;
    ...
    if (!rob_bone_data->motion_loaded.size)
        return;
    if (motion_blend_type == MOTION_BLEND_FREEZE) {
...
static void sub_14041D2A0(rob_chara_bone_data* a1, float_t a2) {
    a1->motion_loaded.head->next->value->field_4F8.prev_rot_y = a2;
}

static void rob_chara_bone_data_set_rot_y(rob_chara_bone_data* rob_bone_data, float_t rot_y) {
    rob_bone_data->motion_loaded.head->next->value->bone_data.rot_y = rot_y;
}
...
static void motion_blend_mot_set_duration(motion_blend_mot* a1,
    float_t duration, float_t step, float_t offset) {
    MotionBlend* v4 = a1->blend;
    if (v4)
        sub_1403FBED0(v4, duration, step, offset);
}

static void rob_chara_bone_data_set_motion_duration(rob_chara_bone_data* a1,
    float_t duration, float_t step, float_t offset) {
    motion_blend_mot_set_duration(a1->motion_loaded.head->next->value, duration, step, offset);
}

static void sub_14041D310(rob_chara_bone_data* a1, float_t a2, float_t a3, int32_t a4) {
    MotionBlend* v1 = a1->motion_loaded.head->next->value->blend;
    if (v1)
        v1->__vftable->Field10(v1, a2, a3, a4);
}
...
static void rob_chara_load_default_motion_sub(rob_chara* a1, int32_t a2, int32_t motion_id) {
    ...
    rob_chara_bone_data_set_rot_y(a1->bone_data, 0.0f);
    rob_chara_bone_data_set_motion_duration(a1->bone_data, 0.0f, 1.0f, 1.0f);
    sub_14041D310(a1->bone_data, 0.0f, 0.0f, 2);
...
int32_t rob_cmn_mottbl_get_motion_index(rob_chara* a1, int32_t a2) {
    ...
    if (a2 == 224)
        return a1->data.motion.motion_id;
    if (a2 >= 214 && a2 <= 223)
...
static char* ExNodeBlock_get_parent_name(ExNodeBlock* a1) {
...
static void rob_chara_item_equip_object_get_parent_bone_nodes(
    ...
    a1->bone_nodes = a2;
    a1->mat = a2->mat;
    for (ExNodeBlock** i = a1->node_blocks.begin; i != a1->node_blocks.end; i++)
        (*i)->parent_bone_node = rob_chara_item_equip_object_get_bone_node_by_name(
            a1, ExNodeBlock_get_parent_name(*i));
}
...
static void rob_chara_bone_data_ik_scale_calculate(
    ...
    float_t base_height = fabsf(b_cl_momo_l->base_translation[0].y) + b_cl_momo_l->ik_segment_length[0]
        + b_cl_momo_l->ik_2nd_segment_length[0] + bone_database_get_skeleton_heel_height(base_skeleton_type);
    float_t height = fabsf(b_cl_momo_l->base_translation[1].y) + b_cl_momo_l->ik_segment_length[1]
        + b_cl_momo_l->ik_2nd_segment_length[1] + bone_database_get_skeleton_heel_height(skeleton_type);
...
static object_info sub_140525C00(rob_chara_item_cos_data* a1, int32_t a2) {
    tree_pair_int32_t_object_info_node* v2 = a1->head_replace.head;
    tree_pair_int32_t_object_info_node* v3 = v2;
    tree_pair_int32_t_object_info_node* v4 = v2->parent;
    while (!v4->is_null)
        if (v4->key >= a2) {
            v3 = v4;
            v4 = v4->left;
    ...
    tree_pair_int32_t_object_info_node* v6;
    if (v3 == v2 || a2 < v3->key)
        v6 = v2;
    else
        v6 = v3;
...
static object_info sub_1405104C0(rob_chara* a1, int32_t a2) {
    if (a2 > 8)
        return object_info_null;

    object_info v3 = sub_140525C00(&a1->item_cos_data, a2);
...
void rob_chara_reset_data(rob_chara* a1, rob_chara_pv_data* a2) {
    ...
    rob_chara_bone_data_init_data(a1->bone_data,
        BONE_DATABASE_SKELETON_COMMON, a1->chara_init_data->skeleton_type);
    ...
    v3->miku_rot.rot_y_int16 = a2->rot_y_int16;
    v3->miku_rot.field_6 = a2->rot_y_int16;
    v3->miku_rot.position = v8;
    ...
    v3->motion.field_138 = ratio0;
    v3->motion.field_13C = ratio0;
    v3->motion.field_140 = 0;
    v3->motion.field_144 = 0;
    v3->motion.field_148 = 0;
    rob_chara_load_default_motion(a1);
    v3->field_8.field_0 = 0;
    v3->field_8.field_4.field_0 = 1;
    v3->field_8.field_4.motion_id = rob_cmn_mottbl_get_motion_index(a1, 0);
    v3->field_8.field_4.field_10 = a2->field_5;
    rob_chara_load_motion(a1, v3->field_8.field_4.motion_id, v3->field_8.field_4.field_10, 0.0, 0);
    if (!a2->field_6)
    ...
    object_info v17 = sub_1405104C0(a1, 0);
    v3->motion.field_150.head_object = v17;
    v3->motion.field_3B0.head_object = v17;
