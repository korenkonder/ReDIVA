...
struct EffectAuth3d {
    size_t count;
    size_t max_count;
    auth_3d_detail::Handle* auth_3d_ids_ptr;
    auth_3d_detail::Handle auth_3d_ids[TASK_STAGE_STAGE_COUNT];
...
struct EffectFogRing {
    ...
    static void draw_static(render_data_context& rend_data_ctx, void* data, const cam_data& cam);
...
bool TaskEffectAuth3D::init() {
    ...
    if (stage.count) {
        for (int32_t i = 0; i < stage.count; i++)
            if (!stage.auth_3d_ids_ptr[i].load_is_done())
                return false;
    }

    for (int32_t i = 0; i < stage.count; i++) {
        auth_3d_detail::Handle& id = stage.auth_3d_ids_ptr[i];
        id.set_looped(true);
        id.set_pause(false);
        id.set_enabled(true);
    }
    ...
    for (uint32_t i : this->stage_hashes) {
        ...
        if (stage.count != stage.max_count)
            for (uint32_t j : stg_data->auth_3d_ids) {
                ...
                auth_3d_detail::Handle id = auth_3d_detail::Handle::create(j, data, obj_db, tex_db);
                if (!id.is_valid())
                    break;
    ...
    for (int32_t i : this->stage_indices) {
        ...
        if (stage.count != stage.max_count)
            for (int32_t j : stage_data->auth_3d_ids) {
                ...
                auth_3d_detail::Handle id = auth_3d_detail::Handle::create(j, aft_auth_3d_db);
                if (!id.is_valid())
                    break;

                id.load_req(aft_auth_3d_db);
                id.set_visible(false);
                if (stage.count + 1 <= stage.max_count)
                    stage.auth_3d_ids_ptr[stage.count++] = id;
                v30.auth_3d_ids.push_back(id);
...
void TaskEffectAuth3D::set_frame(int32_t value) {
    ...
    for (int32_t i = 0; i < stage.count; i++) {
        auth_3d_detail::Handle& id = stage.auth_3d_ids_ptr[i];
        float_t frame_size = id.get_frame_size();
        float_t frame_req;
        if (frame_size != 0.0f)
            frame_req = (float_t)fmod(frame, frame_size);
        else
            frame_req = 0.0f;
        id.set_frame_req(frame_req);
    }
}
...
void TaskEffectAuth3D::set_frame_rate_control(FrameRateControl* value) {
    for (int32_t i = 0; i < stage.count; i++) {
        auth_3d_detail::Handle& id = stage.auth_3d_ids_ptr[i];
        id.set_frame_rate_control(value);
    }
}
...
void TaskEffectAuth3D::set_visible(bool value) {
...
void EffectFogRing::draw_static(render_data_context& rend_data_ctx, void* data, const cam_data& cam) {
...
void EffectRipple::draw_static(render_data_context& rend_data_ctx, void* data, const cam_data& cam) {
...
void star_catalog::draw(render_data_context& rend_data_ctx, const cam_data& cam) {
    ...
    if (milky_way_tex)
        milky_way.draw(rend_data_ctx, vp, model, milky_way_tex, scene_ubo);
...
bool star_catalog::init() {
    ...
    scene_ubo.Create(gl_state, sizeof(star_catalog_scene_shader_data));
    batch_ubo.Create(gl_state, sizeof(star_catalog_batch_shader_data));
    return true;
}
...
static void leaf_particle_init(bool change_stage) {
    ...
    gl_state.bind_array_buffer(leaf_ptcl_vbo);
...
static void particle_init(vec3* offset) {
    ...
    gl_state.bind_array_buffer(ptcl_vbo);
