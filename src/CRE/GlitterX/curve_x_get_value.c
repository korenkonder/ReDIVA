...
static float_t FASTCALL glitter_x_curve_interpolate(glitter_curve* c, float_t frame,
    glitter_curve_key* curr, glitter_curve_key* next, glitter_key_type key_type, glitter_random* random);
static float_t FASTCALL glitter_x_curve_randomize(glitter_curve* c,
    float_t value, glitter_random* random);
static float_t FASTCALL glitter_x_curve_randomize_key(glitter_curve* c,
    glitter_curve_key* key, glitter_random* random);

bool FASTCALL glitter_x_curve_get_value(glitter_curve* c,
    ...
    random_val = glitter_x_random_get_value(random);
    glitter_x_random_set_value(random, random_value);
    negate = c->flags & GLITTER_CURVE_NEGATE
        && glitter_x_random_get_int_min_max(random, 0, 0xFFFF) > 0x7FFF;
    if (c->flags & GLITTER_CURVE_STEP)
        glitter_x_random_set_value(random, random_val + 1);
...
End:
    _value = glitter_x_curve_randomize(c, _value, random);
...
static float_t FASTCALL glitter_x_curve_interpolate(glitter_curve* c, float_t frame,
    ...
    if (key_type == GLITTER_KEY_CONSTANT)
        return glitter_x_curve_randomize_key(c, curr, random);
    ...
    if (key_type == GLITTER_KEY_HERMITE) {
        ...
        val = glitter_x_curve_randomize_key(c, curr, random);
