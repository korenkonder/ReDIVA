...
namespace GL {
    void UniformBuffer::Create(gl_state_struct& gl_st, size_t size, BufferUsage usage) {
        ...
        if (usage != BUFFER_USAGE_STREAM) {
            if (GLAD_GL_VERSION_4_4) {
                ...
                if (GLAD_GL_VERSION_4_5) {
                    ...
                    glBufferStorage(GL_UNIFORM_BUFFER, (GLsizeiptr)size, 0, flags);
                }
                return;
            }
        }
        ...
        if (GLAD_GL_VERSION_4_5) {
            ...
            glGenBuffers(1, &buffer);
            gl_st.bind_uniform_buffer(buffer, true);
            glBufferData(GL_UNIFORM_BUFFER, (GLsizeiptr)size, 0, BufferUsageToGLenum(usage));
        }
    }

    void UniformBuffer::Create(gl_state_struct& gl_st, size_t size, const void* data, BufferUsage usage) {
        ...
        if (usage != BUFFER_USAGE_STREAM) {
            if (GLAD_GL_VERSION_4_4) {
                ...
                if (GLAD_GL_VERSION_4_5) {
                    ...
                    glGenBuffers(1, &buffer);
                    gl_st.bind_uniform_buffer(buffer, true);
                    glBufferStorage(GL_UNIFORM_BUFFER, (GLsizeiptr)size, data, flags);
                }
                return;
            }
        }
        ...
        if (GLAD_GL_VERSION_4_5) {
            ...
            glGenBuffers(1, &buffer);
            gl_st.bind_uniform_buffer(buffer, true);
            glBufferData(GL_UNIFORM_BUFFER, (GLsizeiptr)size, data, BufferUsageToGLenum(usage));
        }
    }
    ...
    void UniformBuffer::Recreate(gl_state_struct& gl_st, size_t size, BufferUsage usage) {
        if (GLAD_GL_VERSION_4_4 && usage != BUFFER_USAGE_STREAM) {
            ...
            if (GLAD_GL_VERSION_4_5)
                ...
                gl_st.bind_uniform_buffer(buffer);
                glBufferData(GL_UNIFORM_BUFFER, (GLsizeiptr)size, 0, usage);
            }
        }
    }

    void UniformBuffer::Recreate(gl_state_struct& gl_st, size_t size, const void* data, BufferUsage usage) {
        if (GLAD_GL_VERSION_4_4 && usage != BUFFER_USAGE_STREAM) {
            Destroy();
