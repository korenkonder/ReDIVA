...
glitter_file_reader* FASTCALL glitter_file_reader_init(wchar_t* path, wchar_t* file, bool f2) {
    ...
    fr->hash = f2 ? hash_wchar_t_murmurhash(file, 0, false)
...
bool FASTCALL Glitter__FileReader__Read(glitter_file_reader* fr) {
    farc* f = farc_init();
    wchar_t* path_temp0;
    wchar_t* path_temp1;
    ...
    free(path_temp0);
    bool ret = Glitter__DivaEffect__ParseFile(fr, (void*)ff->data);
    if (ret) {
        path_temp0 = path_wadd_extension(fr->file, L".drs");
        farc_file* ff = farc_wread_file(f, path_temp0);
        free(path_temp0);
        ret = Glitter__DivaResource__ParseFile(fr->effect_group, (void*)ff->data);
    }
