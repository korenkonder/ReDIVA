...
glitter_locus_history* glitter_locus_history_init(size_t size) {
    ...
    vector_old_glitter_locus_history_data_reserve(&lh->data, size);
    return lh;
}
...
void glitter_locus_history_append(glitter_locus_history* a1,
    ...
    data = a1->data.begin;
    size = vector_old_length(a1->data);
    temp = a2->translation;
    ...
    if (size < 1)
        vector_old_glitter_locus_history_data_push_back(&a1->data, &locus_history);
    else if (size == 1) {
        locus_history.translation = data->translation;
        if (vector_old_capacity(a1->data) > 1)
            vector_old_glitter_locus_history_data_push_back(&a1->data, &locus_history);
        ...
        if (size < vector_old_capacity(a1->data)) {
...
void glitter_locus_history_dispose(glitter_locus_history* lh) {
    vector_old_glitter_locus_history_data_free(&lh->data, 0);
    free(lh);
}
