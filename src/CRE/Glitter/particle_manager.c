...
bool GltParticleManager::Ctrl() {
    if (flags & GLITTER_PARTICLE_MANAGER_READ_FILES) {
        for (std::vector<GlitterFileReader*>::iterator i = file_readers.begin(); i != file_readers.end();)
...
bool GltParticleManager::AppendEffectGroup(uint64_t hash, GlitterEffectGroup* eff_group, GlitterFileReader* file_read) {
...
void GltParticleManager::BasicEffectGroups() {
    for (std::map<uint64_t, GlitterEffectGroup*>::iterator i = effect_groups.begin(); i != effect_groups.end(); ) {
        ...
        for (std::vector<GlitterScene*>::iterator j = scenes.begin(); j != scenes.end();) {
            scene = *j;
...
bool GltParticleManager::CheckNoFileReaders(uint64_t hash) {
    std::map<uint64_t, GlitterEffectGroup*>::iterator elem = effect_groups.find(hash);
    if (elem != effect_groups.end()) {
        if (file_readers.size())
            return true;

        for (GlitterFileReader*& i : file_readers)
            if (i && i->hash == hash)
                return false;
        return true;
    }
...
void GltParticleManager::CtrlScenes() {
    for (std::vector<GlitterScene*>::iterator i = scenes.begin(); i != scenes.end();) {
...
#if defined(CRE_DEV)
        else if (~scene->flags & GLITTER_SCENE_EDITOR) {
...
void GltParticleManager::FreeSceneEffect(GlitterSceneCounter scene_counter, uint64_t hash, bool force_kill) {
    if (scene_counter) {
        for (std::vector<GlitterScene*>::iterator i = scenes.begin(); i != scenes.end();) {
            GlitterScene* scene = *i;
...
void GltParticleManager::FreeScenes() {
    for (GlitterScene*& i : scenes)
        if (i)
            i->FreeEffect(this, i->type == GLITTER_FT
...
GlitterEffectGroup* GltParticleManager::GetEffectGroup(uint64_t hash) {
    std::map<uint64_t, GlitterEffectGroup*>::iterator elem = effect_groups.find(hash);
    if (elem != effect_groups.end())
        return elem->second;
    return 0;
...
const char* GltParticleManager::GetEffectName(uint64_t hash, int32_t index) {
    ...
    GlitterEffectGroup* eff_group = elem->second;
    if (index >= eff_group->effects.size())
        return 0;
    return eff_group->effects.data()[index]->name;
}
...
GlitterScene* GltParticleManager::GetScene(uint64_t hash) {
    for (GlitterScene*& i : scenes)
        if (i && i->hash == hash)
            return i;
    return 0;
...
GlitterScene* GltParticleManager::GetScene(GlitterSceneCounter scene_counter) {
    for (GlitterScene*& i : scenes)
...
float_t GltParticleManager::GetSceneFrameLifeTime(GlitterSceneCounter scene_counter, int32_t* life_time) {
    if (life_time)
        *life_time = 0;
    ...
    float_t frame = 0.0f;
    if (life_time)
        for (GlitterSceneEffect& i : scene->effects) {
            if (!i.disp || !i.ptr)
                continue;

            GlitterEffectInst* eff_inst = i.ptr;
...
GlitterSceneCounter GltParticleManager::GetSceneCounter(uint8_t index) {
    if (scene_load_counter + 1 < 0xFFFFFF && (~flags & GLITTER_PARTICLE_MANAGER_RESET_SCENE_COUNTER)) {
        scene_load_counter++;
        return GlitterSceneCounter(index, scene_load_counter);
    }
    ...
    if (counter < 0xFFFFFF)
        ...
        for (uint32_t v7 = 1; ; v7++) {
            ...
            if (v7 >= 300)
                return GlitterSceneCounter(0);
        }
    }
    return GlitterSceneCounter(index, counter);
}

GlitterSceneCounter GltParticleManager::Load(uint64_t effect_group_hash, uint64_t effect_hash, bool use_existing) {
    if (effect_group_hash == hash_fnv1a64m_empty || effect_group_hash == hash_murmurhash_empty) {
        ...
        bool found = false;
        for (std::pair<uint64_t, GlitterEffectGroup*> i : effect_groups) {
            for (glitter_effect*& j : i.second->effects)
    ...
    std::map<uint64_t, GlitterEffectGroup*>::iterator elem = effect_groups.find(effect_group_hash);
    if (elem == effect_groups.end())
        return GlitterSceneCounter(0);
    ...
    if (use_existing)
        for (GlitterScene*& i : scenes) {
            ...
            int32_t id = 1;
            if (effect_hash == hash_fnv1a64m_empty || effect_hash == hash_murmurhash_empty)
                for (glitter_effect*& j : eff_group->effects)
    ...
    GlitterSceneCounter counter = GetSceneCounter(0);
    if (!counter)
        return GlitterSceneCounter(0);

    GlitterScene* scene = new GlitterScene(counter, effect_group_hash, eff_group, 0);
    if (!scene)
        return GlitterSceneCounter(0);

    int32_t id = 1;
    if (effect_hash == hash_fnv1a64m_empty || effect_hash == hash_murmurhash_empty) {
        for (glitter_effect*& i : eff_group->effects)
            if (i)
...
uint64_t GltParticleManager::LoadFile(GLT, void* data, const char* file,
    ...
    GlitterFileReader* fr = new GlitterFileReader(GLT_VAL);
    if (!fr)
        return empty_hash;
...
GlitterSceneCounter GltParticleManager::LoadScene(uint64_t effect_group_hash, uint64_t effect_hash, bool appear_now) {
    ...
    GlitterSceneCounter counter = GetSceneCounter(0);
    if (counter) {
        GlitterScene* scene = new GlitterScene(counter, effect_group_hash, eff_group, false);
        if (scene) {
            int32_t id = 1;
            for (glitter_effect*& i : eff_group->effects)
                if (i)
...
GlitterSceneCounter GltParticleManager::LoadSceneEffect(uint64_t hash, bool appear_now) {
    ...
    for (std::pair<uint64_t, GlitterEffectGroup*> i : effect_groups) {
        GlitterEffectGroup* v8 = i.second;

        if (v8->effects.size() < 1)
            continue;

        bool found = false;
        for (glitter_effect*& j : v8->effects)
        ...
        for (GlitterScene*& j : scenes)
            if (j && j->hash == i.first) {
                size_t id = 1;
                for (glitter_effect*& k : v8->effects) {
                    if (!j)
                        continue;
        ...
        GlitterScene* scene = new GlitterScene(counter, i.first, i.second, 0);
        if (!scene)
            return GlitterSceneCounter(0);
...
bool GltParticleManager::SceneHasNotEnded(GlitterSceneCounter counter) {
    for (GlitterScene*& i : scenes)
...
#if defined(CRE_DEV)
void GltParticleManager::SetFrame(GlitterEffectGroup* effect_group,
