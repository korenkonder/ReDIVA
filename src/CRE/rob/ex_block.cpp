...
static int32_t sub_140485180(osage_coli* a1, vec3* a2, float_t& coli_r);
static int32_t sub_1404851C0(vec3* a1, float_t& coli_r, std::vector<osage_coli>* a3, float_t* a4);
static int32_t sub_140485220(vec3* a1, float_t& coli_r, osage_coli* a3, float_t* a4);
...
static void sub_140484E10(vec3* a1, vec3* a2, vec3* a3, vec3* a4);
...
void osage_coli::set(osage_coli* coli, skin_param_osage_root_coli* skp_root_coli, mat4* mats) {
    ...
    for (; skp_root_coli->type; coli++, skp_root_coli++) {
        ...
        if (skp_root_coli->type == SKIN_PARAM_OSAGE_ROOT_COLI_TYPE_CYLINDER
            ...
            if (coli->bone_pos_diff_length < 0.01f)
...
void osage_coli::ring_set(osage_coli* coli,
    ...
    if (!vec_skp_root_coli.size()) {
        ...
        set(coli, vec_skp_root_coli.data(), mats);
}
...
tangent(), binormal(), tangent_sign(), texcoord(), field_64(), length(), field_74(),
field_78(), field_7C(), field_80(), field_88(), reset_data(), field_B4(), field_D0() {
...
void RobCloth::AddMotionResetData(int32_t motion_id, float_t frame) {
    ...
    std::list<RobOsageNodeResetData> v28;
    ...
    motion_reset_data.insert({ { motion_id, frame_int }, v28 });
}

void RobCloth::ColiSet(mat4* mats) {
    if (skin_param_ptr->coli.size())
        osage_coli::set(coli, skin_param_ptr->coli.data(), mats);
    osage_coli::ring_set(coli_ring, ring.skp_root_coli, mats);
}
...
void RobCloth::InitData(size_t root_count, size_t nodes_count, obj_skin_block_cloth_root* root,
    ...
    for (size_t i = 1; i < nodes_count; i++)
        for (size_t j = 0; j < root_count; j++) {
            ...
            v72.length = node.length;
            v72.field_74 = node.field_20;
            v72.field_78 = node.field_24;
            v72.field_7C = node.field_28;
            v72.reset_data = {};
...
void RobCloth::SetMotionResetData(int32_t motion_id, float_t frame) {    osage_reset = true;
...
void RobCloth::SetOsagePlayData(std::vector<opd_blend_data>& opd_blend_data) {
    ...
    size_t v7 = opd_blend_data.size() - 1;
    for (auto i = opd_blend_data.end(); i != opd_blend_data.begin(); v7--) {
        i--;
        ...
        for (size_t j = 0; j < root_count; j++) {
            ...
            for (size_t k = 1; k < nodes_count; k++, v29 += root_count) {
                opd_vec3_data* opd = &v29->opd_data[v7];
                float_t* opd_x = opd->x;
                float_t* opd_y = opd->y;
                float_t* opd_z = opd->z;
                ...
                v83.length = v29->length;
                sub_140482100(&v29->field_D0, &i[0], &v83);
...
void RobOsage::SetOsagePlayData(mat4* parent_mat,
    ...
    sub_14047F110(this, &v85, &parent_scale, 0);
    *nodes.data()[0].bone_node_mat = v85;
    *nodes.data()[0].bone_node_ptr->ex_data_mat = v85;

    size_t v17 = opd_blend_data.size() - 1;
    for (auto i = opd_blend_data.end(); i != opd_blend_data.begin(); v17--) {
        i--;
    ...
    RobOsageNode* i_end = nodes.data() + nodes.size();
    for (RobOsageNode* i = i_begin; i != i_end; i++) {
        float_t rot_y = i->field_1B0.field_0.rotation.y;
        float_t rot_z = i->field_1B0.field_0.rotation.z;
    ...
    if (rob_cls->move_cancel > 0.0f) {
        ...
        for (size_t i = 0; i < root_count; i++) {
            ...
            for (size_t j = 1; j < nodes_count; j++, node += root_count) {
                ...
                vec3 v20;
                vec3_sub(v19, node->trans, v20);
                vec3_mult_scalar(v20, move_cancel, v20);
                vec3_add(node->trans, v20, node->trans);
            }
        }
    }
...
static void sub_140219940(RobCloth* rob_cls) {
    ...
    for (size_t i = 0; i < root_count; i++) {
        ...
        for (int32_t j = 0; j < 4; j++) {
            ...
            mat4 mat = *root.mat[j];
            mat4_mult(&mat, root.node_mat[j], &mat);
        ...
        mat4_mult_vec3(&m, &rob_cls->root.data()[i].normal, &root_node.normal);
        mat4_mult_vec3(&m, (vec3*)&rob_cls->root.data()[i].tangent, &root_node.tangent);
        root_node.tangent_sign = rob_cls->root.data()[i].tangent.w;
...
static void sub_140219D10(RobCloth* rob_cls) {
    ...
    size_t nodes_count = rob_cls->nodes_count;
    size_t v4 = nodes_count - 2;

    CLOTHNode* v7 = &node[root_count * (nodes_count - 2)];
    for (size_t i = nodes_count - 2; i; i--) {
        CLOTHNode* v8 = &v7[root_count];
        for (ssize_t j = 0; j < root_count; j++, v8++)
            sub_140482F30(&v8[-root_count].trans, &v8->trans, v8[-root_count].length);
        v7 -= root_count;
    }

    std::vector<vec3> v52;
    std::vector<vec3> v53;
    v52.resize(root_count);
    ...
    for (ssize_t i = nodes_count - 1; i >= 1; i--) {
        vec3* v17 = v12;
        vec3* v17a = v11;
        if (root_count) {
            CLOTHNode* v18 = v15;
            for (ssize_t j = 0; j < root_count; j++, v17++, v17a++, v18++) {
                *v17 = v18->trans;
                *v17a = v18->trans;
            }
        }

        vec3* v20 = v11 + root_count - 2;
        CLOTHNode* v22 = v16 - 1;
        for (ssize_t j = root_count - 2; j; j--, v20--, v22--)
            sub_140482F30(v20, v20 + 1, v22->field_7C);

        if (rob_cls->field_8 & 0x04)
            sub_140482F30(&v11[root_count - 1], v11, v16->field_7C);

        vec3* v23 = v12 + 1;
        CLOTHNode* v24 = v15 + 1;
        for (ssize_t j = 1; j < root_count; j++, v23++, v24++)
            sub_140482F30(v23, v23 - 1, v24->field_7C);

        if (rob_cls->field_8 & 0x04)
            sub_140482F30(v12, &v12[root_count - 1], v15->field_7C);

        CLOTHNode* v26 = v15;
        vec3* v27 = v11;
        vec3* v28 = v12;
        CLOTHNode* v36 = v26;
        for (ssize_t i = 0; i < root_count; i++, v27++, v28++, v36++) {
            vec3 v44;
...
void sub_14021AA60(RobCloth* rob_cls, float_t step, bool a3) {
    ...
    for (size_t i = 1; i < nodes_count; i++) {
        for (ssize_t j = 0; j < root_count; j++, node++) {
            float_t v71 = (1.0f - rob_cls->field_44) * rob_cls->skin_param_ptr->friction;
            if (step != 1.0f) {
                vec3 v60;
            ...
            sub_140482F30(&node[0].trans, &node[-root_count].trans, node[0].length);

            int32_t v39 = sub_1404851C0(&node->trans, rob_cls->skin_param_ptr->coli_r, &rob_cls->ring.coli, &v71);
            v39 += sub_140485180(rob_cls->coli_ring, &node->trans, rob_cls->skin_param_ptr->coli_r);
            v39 += sub_140485180(rob_cls->coli, &node->trans, rob_cls->skin_param_ptr->coli_r);
            ...
            if (v39)
                vec3_mult_scalar(node->trans_diff, v71, node->trans_diff);
...
static void sub_14021D480(RobCloth* rob_cls) {
    ...
    for (size_t i = 1; i < nodes_count; i++) {
        ...
        for (ssize_t j = 0; j < root_count; j++, root++, node++) {
            ...
            vec3_mult_scalar(v38, node->length, v38);
            vec3_add(node[-root_count].trans, v38, node[0].trans);

            sub_1404851C0(&node->trans, rob_cls->skin_param_ptr->coli_r, &rob_cls->ring.coli, 0);
            sub_140485180(rob_cls->coli_ring, &node->trans, rob_cls->skin_param_ptr->coli_r);
            sub_140485180(rob_cls->coli, &node->trans, rob_cls->skin_param_ptr->coli_r);
...
static int32_t sub_140485180(osage_coli* a1, vec3* a2, float_t& coli_r) {
    return sub_140485220(a2, coli_r, a1, 0);
}

static int32_t sub_1404851C0(vec3* a1, float_t& coli_r, std::vector<osage_coli>* a3, float_t* a4) {
    if (!a3->size() || a3->back().type)
        return 0;
    return sub_140485220(a1, coli_r, a3->data(), a4);
}
...
static int32_t sub_140485220(vec3* a1, float_t& coli_r, osage_coli* a3, float_t* a4) {
    ...
    while (a3->type) {
        ...
        case SKIN_PARAM_OSAGE_ROOT_COLI_TYPE_CYLINDER:
            v11 = sub_140484540(&v31, a1, a3, a3->radius + coli_r);
            break;
        case SKIN_PARAM_OSAGE_ROOT_COLI_TYPE_PLANE:
            v11 = sub_140484780(&v31, a1, &a3->bone0_pos, &a3->bone1_pos, coli_r);
            break;
        case SKIN_PARAM_OSAGE_ROOT_COLI_TYPE_ELLIPSE:
            v11 = sub_140483EA0(&v31, a1, a3, a3->radius + coli_r);
            break;
        case SKIN_PARAM_OSAGE_ROOT_COLI_TYPE_5:
            v11 = sub_140483B30(&v31, a1, a3, coli_r);
            break;
        }
    ...
    for (RobOsageNode* v35 = v35_begin; v35 != v35_end; v35++) {
        ...
        if (a5) {
            ...
            if (!rob_osg->field_1F0F) {
                v35->field_C8 += (float_t)sub_140485180(coli_ring, &v35->trans, skp_osg_node->coli_r);
                v35->field_C8 += (float_t)sub_140485180(coli, &v35->trans, skp_osg_node->coli_r);
            }
...
static void sub_14047ECA0(RobOsage* rob_osg, float_t step) {
    ...
    for (RobOsageNode* i = i_begin; i != i_end; i++) {
        RobOsageNodeData* data = i->data_ptr;
        i->field_C8 += (float_t)sub_1404851C0(&i->trans, data->skp_osg_node.coli_r, vec_coli, &i->field_CC);
        if (v11)
            continue;

        for (RobOsageNode*& j : data->boc) {
            j->field_C8 += (float_t)sub_140485180(coli_ring, &j->trans, data->skp_osg_node.coli_r);
            j->field_C8 += (float_t)sub_140485180(coli, &j->trans, data->skp_osg_node.coli_r);
        }

        i->field_C8 += (float_t)sub_140485180(coli_ring, &i->trans, data->skp_osg_node.coli_r);
        i->field_C8 += (float_t)sub_140485180(coli, &i->trans, data->skp_osg_node.coli_r);
    }
}
...
static void sub_14047F990(RobOsage* rob_osg, mat4* a2, vec3* a3, bool a4) {
    ...
    for (RobOsageNode* j = j_begin; j != j_end; i++, j++) {
        ...
        sub_140485180(coli_ring, &v74, v38->coli_r);
        sub_140485180(coli, &v74, v38->coli_r);
