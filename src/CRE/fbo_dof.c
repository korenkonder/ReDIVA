...
extern int32_t dof_common_ubo_binding;
extern int32_t dof_texcoords_ubo_binding;
...
void fbo_dof_draw(fbo_dof* dfbo, int32_t color_tcb, int32_t depth_tcb, bool dof_f2,
    ...
    bind_uniform_buffer_base(dof_common_ubo_binding, dfbo->dof_common_ubo);
    bind_uniform_buffer_base(dof_texcoords_ubo_binding, dfbo->texcoords_ubo);
...
static void fbo_dof_bind_fbo(fbo_dof* dfbo, vec2i* res) {
    ...
    fbo_helper_gen_texture_image(dfbo->tcb[0], dfbo->res_20.x, dfbo->res_20.y,
        GL_RG16F, GL_RG, GL_HALF_FLOAT, 0);
    fbo_helper_get_error_code();

    bind_framebuffer(dfbo->fbo[1]);
    glDrawBuffers(1, fbo_dof_s_attachments);
    fbo_helper_gen_texture_image(dfbo->tcb[1], dfbo->res_20.x, dfbo->res_20.y,
        GL_RG16F, GL_RG, GL_HALF_FLOAT, 0);
    fbo_helper_get_error_code();

    bind_framebuffer(dfbo->fbo[2]);
    glDrawBuffers(2, fbo_dof_d_attachments);
    fbo_helper_gen_texture_image(dfbo->tcb[2], dfbo->res_2.x, dfbo->res_2.y,
        GL_RGBA16F, GL_RGBA, GL_HALF_FLOAT, 0);
    fbo_helper_gen_texture_image(dfbo->tcb[3], dfbo->res_2.x, dfbo->res_2.y,
        GL_R11F_G11F_B10F, GL_RGB, GL_UNSIGNED_INT_10F_11F_11F_REV, 1);
    fbo_helper_get_error_code();

    bind_framebuffer(dfbo->fbo[3]);
    glDrawBuffers(2, fbo_dof_d_attachments);
    fbo_helper_gen_texture_image(dfbo->tcb[4], dfbo->res_2.x, dfbo->res_2.y,
        GL_RGBA16F, GL_RGBA, GL_HALF_FLOAT, 0);
    fbo_helper_gen_texture_image(dfbo->tcb[5], dfbo->res_2.x, dfbo->res_2.y,
