...
static rgb565 rgb565_apply_color_tone(rgb565 col, const ImgfColorToneParam* color);
static void rgb_to_ycc(vec3& rgb, vec3& ycc);
static void ycc_apply_col_tone_hue(vec3& ycc, const ImgfColorToneParam* color);
static void ycc_to_rgb(vec3& ycc, vec3& rgb);

ImgfColorToneParam::ImgfColorToneParam() : reserved() {
    blend_color = 1.0f;
    offset_color = 0.0f;
    hue = 0.0f;
    saturation = 1.0f;
...
void dxt1_image_apply_color_tone(int32_t width, int32_t height,
    ...
    for (size_t i = (size - 1ULL) / sizeof(dxt1_block) + 1; i; i--, data++) {
        data->color0 = rgb565_apply_color_tone(data->color0, color);
        data->color1 = rgb565_apply_color_tone(data->color1, color);
...
void rgb565_image_apply_color_tone(int32_t width, int32_t height,
    ...
    for (size_t i = (size - 1ULL) / sizeof(uint16_t) + 1; i; i--, data++)
        *data = rgb565_apply_color_tone(*data, color);
}
...
static rgb565 rgb565_apply_color_tone(rgb565 col, const ImgfColorToneParam* color) {
    ...
    rgb = vec3::clamp(rgb * inv_scale * color->blend_color + color->offset_color, 0.0f, 1.0f);

    vec3 ycc;
    rgb_to_ycc(rgb, ycc);

    *(vec2*)&ycc.y = *(vec2*)&ycc.y * color->saturation;

    ycc_apply_col_tone_hue(ycc, color);

    ycc.x = (ycc.x + color->value - 0.5f) * color->contrast + 0.5f;
...
inline static void ycc_apply_col_tone_hue(vec3& ycc, const ImgfColorToneParam* color) {
    float_t hue = color->hue * DEG_TO_RAD_FLOAT;
