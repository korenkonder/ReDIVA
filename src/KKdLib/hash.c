...
uint64_t hash_char_fnv1a64(char* data) {
    uint64_t hash = hash_fnv1a64((uint8_t*)data, strlen(data));
    return hash;
}

uint64_t hash_wchar_t_fnv1a64(wchar_t* data) {
    ...
    if (already_upper) {
        if (big_endian)
            for (i = 0; length > 3; length -= 4, i += 4) {
                b = (uint32_t)data[i + 3] | ((uint32_t)data[i + 2] << 8)
                    | ((uint32_t)data[i + 1] << 16) | ((uint32_t)data[i + 0] << 24);
                hash += b;
                hash *= m;
                hash ^= hash >> r;
            }
            ...
            for (i = 0; length > 3; length -= 4, i += 4) {
                b = (uint32_t)data[i + 0] | ((uint32_t)data[i + 1] << 8)
...
uint32_t hash_char_murmurhash(char* data, uint32_t seed, bool already_upper) {
    uint32_t hash = hash_murmurhash((uint8_t*)data, strlen(data), seed, already_upper, false);
    return hash;
}

uint32_t hash_wchar_t_murmurhash(wchar_t* data, uint32_t seed, bool already_upper) {
    char* temp = utf8_encode(data);
