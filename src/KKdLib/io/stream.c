...
void io_open(stream* s, const char* path, const char* mode) {
    ...
    s->position_stack = vector_old_empty(int64_t);
    io_get_length(s);
    free(temp_path);
    free(temp_mode);
}
...
void io_open(stream* s, const void* data, size_t size) {
    memset(s, 0, sizeof(stream));
    if (!data || !size) {
        s->type = STREAM_MEMORY;
        s->length = 0;
    ...
    vector_old_uint8_t_reserve(&s->io.data.vec, size);
    if (s->io.data.vec.begin && data)
        memcpy(s->io.data.vec.begin, data, size);
    s->io.data.data = s->io.data.vec.begin;
    s->io.data.vec.end = s->io.data.vec.begin + size;
    s->type = STREAM_MEMORY;
    s->position_stack = vector_old_empty(int64_t);
    s->length = size;
}
...
void io_copy(stream* s, void** data, size_t* size) {
    if (!s || !data || !size || s->type != STREAM_MEMORY)
        return;

    *size = vector_old_length(s->io.data.vec);
    *data = force_malloc(*size);
    memcpy(*data, s->io.data.vec.begin, *size);
}
...
void io_align_read(stream* s, int64_t align) {
    int64_t position;
    switch (s->type) {
    case STREAM_FILE:
    ...
    int64_t capacity;
    int64_t temp_align = align - position % align;
    if (align != temp_align)
        switch (s->type) {
        case STREAM_FILE:
...
void io_align_write(stream* s, int64_t align) {
    int64_t position;
    switch (s->type) {
    case STREAM_FILE:
        position = _ftelli64(s->io.stream);
...
inline int64_t io_get_length(stream* s) {
    switch (s->type) {
    case STREAM_FILE:
        if (s->io.stream) {
...
inline int64_t io_get_position(stream* s) {
    switch (s->type) {
    case STREAM_FILE:
...
int32_t io_set_position(stream* s, int64_t pos, int32_t seek) {
    int64_t capacity;
    switch (s->type) {
    case STREAM_FILE:
...
inline int32_t io_position_push(stream* s, int64_t pos, int32_t seek) {
    *vector_old_int64_t_reserve_back(&s->position_stack) = io_get_position(s);
...
inline void io_position_pop(stream* s) {
    ...
    int64_t position = s->position_stack.end[-1];
    vector_old_int64_t_pop_back(&s->position_stack, 0);
    io_set_position(s, position, SEEK_SET);
    io_flush(s);
}

inline int64_t io_read(stream* s, int64_t count) {
    return io_read(s, (void*)0, count);
}

int64_t io_read(stream* s, void* buf, int64_t count) {
    int64_t capacity;
    switch (s->type) {
    case STREAM_FILE:
        if (!buf) {
...
inline int64_t io_write(stream* s, int64_t count) {
    return io_write(s, (void*)0, count);
}

int64_t io_write(stream* s, void* buf, int64_t count) {
    int64_t capacity;
    switch (s->type) {
    case STREAM_FILE:
        if (!buf) {
    ...
    case STREAM_MEMORY:
        capacity = s->io.data.vec.capacity_end - s->io.data.data;
        if (capacity < count) {
            int64_t pos = s->io.data.data - s->io.data.vec.begin;
...
inline int64_t io_write(stream* s, const void* buf, int64_t count) {
...
inline void io_read_string_null_terminated(stream* s, string* str) {
    int64_t offset = io_get_position(s);
    int64_t length = 0;
...
inline void io_read_wstring_null_terminated_offset(stream* s,
    int64_t offset, std::wstring* str) {
    if (offset) {
        int64_t length = 0;
        wchar_t* temp = io_read_utf16_string_null_terminated_offset_length(s, offset, &length);
        *str = std::wstring(temp, length);
...
inline char* io_read_utf8_string_null_terminated_length(stream* s, int64_t* length) {
    int64_t offset = io_get_position(s);
    return io_read_utf8_string_null_terminated_offset_length(s, offset, length);
}

inline wchar_t* io_read_utf16_string_null_terminated_length(stream* s, int64_t* length) {
    int64_t offset = io_get_position(s);
    return io_read_utf16_string_null_terminated_offset_length(s, offset, length);
}

inline char* io_read_utf8_string_null_terminated_offset(stream* s, int64_t offset) {
    int64_t length = 0;
    return io_read_utf8_string_null_terminated_offset_length(s, offset, &length);
}

inline wchar_t* io_read_utf16_string_null_terminated_offset(stream* s, int64_t offset) {
    int64_t length = 0;
    return io_read_utf16_string_null_terminated_offset_length(s, offset, &length);
}

char* io_read_utf8_string_null_terminated_offset_length(stream* s, int64_t offset, int64_t* length) {
...
wchar_t* io_read_utf16_string_null_terminated_offset_length(stream* s, int64_t offset, int64_t* length) {
    io_position_push(s, offset, SEEK_SET);
...
inline int64_t io_read_offset(stream* s, int32_t offset, bool is_x) {
    int64_t val;
    if (!is_x) {
...
inline int64_t io_read_offset_f2(stream* s, int32_t offset) {
    int64_t val = io_read_int32_t_stream_reverse_endianness(s);
    if (val)
        val -= offset;
    return val;
}
...
inline int64_t io_read_offset_x(stream* s) {
    io_align_read(s, 0x08);
    int64_t val = io_read_int64_t_stream_reverse_endianness(s);
    return val;
}

inline void io_write_offset(stream* s, int64_t val, int32_t offset, bool is_x) {
    if (!is_x) {
        if (val)
            val += offset;
...
inline void io_write_offset_f2(stream* s, int64_t val, int32_t offset) {
    if (val)
        val += offset;
    io_write_int32_t_stream_reverse_endianness(s, (int32_t)val);
}

inline void io_write_offset_x(stream* s, int64_t val) {
    io_align_write(s, 0x08);
    io_write_int64_t_stream_reverse_endianness(s, val);
}
...
void io_free(stream* s) {
    ...
    vector_old_int64_t_free(&s->position_stack, 0);
    memset(s, 0, sizeof(stream));
}
