...
static void motion_database_write_inner(motion_database* mot_db, stream* s) {
    ...
    int64_t motion_sets_offset = io_get_position(s);
    ...
    int64_t motion_set_name_offset = io_get_position(s);
    io_write(s, size);
    ...
    int64_t motion_name_offset = io_get_position(s);
    io_write(s, size);
    io_align_write(s, 0x20);

    int64_t motion_names_offset = io_get_position(s);
    size = 0;
    for (motion_set_info& i : mot_db->motion_set)
    ...
    int64_t motion_ids_offset = io_get_position(s);
    for (motion_set_info& i : mot_db->motion_set)
        for (motion_info& j : i.motion)
            io_write_uint32_t(s, j.id);
    io_align_write(s, 0x20);

    int64_t motion_set_ids_offset = io_get_position(s);
    for (motion_set_info& i : mot_db->motion_set)
        io_write_uint32_t(s, i.id);

    int64_t bone_name_offset = io_get_position(s);
    size = 0;
    ...
    int64_t bone_name_offsets_offset = io_get_position(s);
