...
namespace divafile {
    ...
    void decrypt(const wchar_t* path) {
        ...
        if (s_enc.io.stream) {
            ...
            if (signature == 0x454C494641564944) {
                ...
                s_dec.write(data, min(file_length, stream_length));
                free(data);
            }
        }
        free(file_temp);
    ...
    void decrypt(stream& s) {
        ...
        uint64_t signature = s.read_uint64_t();
        if (signature != 0x454C494641564944) {
            s.set_position(pos, SEEK_SET);
            return;
        }

        uint32_t stream_length = s.read_uint32_t();
        uint32_t file_length = s.read_uint32_t();
        void* data = force_malloc(stream_length);
        s.read(data, stream_length);

        aes128_ctx ctx;
        aes128_init_ctx(&ctx, key);
        aes128_ecb_decrypt_buffer(&ctx, (uint8_t*)data, stream_length);

        s.open(data, file_length);
        free(data);
    }
