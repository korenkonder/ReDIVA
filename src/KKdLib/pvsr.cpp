...
static void pvsr_read_inner(pvsr* sr, stream* s) {
    ...
    int64_t x10 = io_read_offset_x(&s_pvsr);
    int64_t stage_effect_offset = io_read_offset_x(&s_pvsr);
    int64_t stage_change_effect_offset = io_read_offset_x(&s_pvsr);
    int64_t stage_effect_env_offset = io_read_offset_x(&s_pvsr);
    ...
    int64_t unknown_offset = io_read_offset_x(&s_pvsr);
    int64_t effect_offset = io_read_offset_x(&s_pvsr);
    int64_t emcs_offset = io_read_offset_x(&s_pvsr);
    ...
    for (int32_t i = 0; i < PVSR_STAGE_CHANGE_EFFECT_COUNT; i++)
        for (int32_t j = 0; j < PVSR_STAGE_CHANGE_EFFECT_COUNT; j++) {
            int64_t offset = io_read_offset_x(&s_pvsr);
            if (offset <= 0)
                continue;
...
static void pvsr_stage_effect_read(pvsr_stage_effect* stg_eff, stream* s) {
    ...
    int64_t a3da_offset = io_read_offset_x(s);
    int64_t glitter_offset = io_read_offset_x(s);
    io_read_offset_x(s);
...
static void pvsr_stage_effect_env_read(pvsr_stage_effect_env* aet, stream* s, int32_t x00) {
    int64_t set_name_offset = io_read_offset_x(s);
    int64_t front_offset = io_read_offset_x(s);
    int64_t front_low_offset = io_read_offset_x(s);
    int64_t back_offset = io_read_offset_x(s);
    int64_t sub1_offset = io_read_offset_x(s);
    int64_t sub2a_offset = io_read_offset_x(s);
    int64_t sub2b_offset = io_read_offset_x(s);
    int64_t sub2c_offset = io_read_offset_x(s);
    int64_t sub2d_offset = io_read_offset_x(s);
    ...
    int8_t front_count = io_read_int8_t(s);
    int8_t front_low_count = io_read_int8_t(s);
    int8_t back_count = io_read_int8_t(s);
    io_align_read(s, 0x08);
    int64_t o68 = 0;
    int64_t o70 = 0;
    int8_t u78 = 0;
    ...
    if (x00 & 0x100) {
        ...
        u78 = io_read_int8_t(s);
        u79 = io_read_int8_t(s);
        io_align_read(s, 0x08);
    }
