...
extern uint64_t hash_fnv1a64m(const void* data, size_t length, bool make_upper = false);
...
extern uint64_t hash_string_fnv1a64m(const string* data, bool make_upper = false);
extern uint64_t hash_string_fnv1a64m(const std::string* data, bool make_upper = false);
extern uint32_t hash_murmurhash(const void* data, size_t length, uint32_t seed = 0, bool upper = false, bool big_endian = false);
...
extern uint32_t hash_string_murmurhash(const string* data, uint32_t seed = 0, bool upper = false);
extern uint32_t hash_string_murmurhash(const std::string* data, uint32_t seed = 0, bool upper = false);
extern uint16_t hash_crc16_ccitt(const void* data, size_t length, bool make_upper = false);
...
extern uint16_t hash_string_crc16_ccitt(const string* data, bool make_upper = false);
