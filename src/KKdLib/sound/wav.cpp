...
static void wav_read_inner(wav* w, stream& s, float_t*& data, size_t& samples);
static void wav_write_inner(wav* w, stream& s, const float_t* data, size_t samples);

wav::wav() : size(), sample_rate(), header_size(),
channel_mask(), is_supported(), bytes(), format(), channels() {
...
void wav::read(const char* path, float_t*& data, size_t& samples) {
    data = 0;
    samples = 0;
    ...
    s.open(path, "rb");
    if (s.check_not_null())
        wav_read_inner(this, s, data, samples);
}

void wav::read(const wchar_t* path, float_t*& data, size_t& samples) {
    data = 0;
    samples = 0;
...
void wav::write(const char* path, const float_t* data, size_t samples) {
    if (!data || !samples || !path)
        return;

    file_stream s;
    s.open(path, "wb");
    if (s.check_not_null())
        wav_write_inner(this, s, data, samples);
}

void wav::write(const wchar_t* path, const float_t* data, size_t samples) {
    if (!data || !samples || !path)
        return;
    ...
    if (w->bytes == 0x01 && w->format == 0x01)
        for (size_t i = 0; i < t_s; i++)
            for (size_t c = 0; c < ch; c++) {
                ...
                sample = div_min_max_def(sample, -128.0, 127.0);
        ...
        for (size_t i = 0; i < t_s; i++)
            for (size_t c = 0; c < ch; c++) {
                ...
                sample = div_min_max_def(sample, -32768.0, 32767.0);
                t_d[i * ch + c] = (float_t)sample;
            }
        ...
        for (size_t i = 0; i < t_s; i++)
            for (size_t c = 0; c < ch; c++) {
                ...
                sample = div_min_max_def(sample, -8388608.0, 8388607.0);
                t_d[i * ch + c] = (float_t)sample;
            }
        ...
        for (size_t i = 0; i < t_s; i++)
            for (size_t c = 0; c < ch; c++) {
                ...
                sample = div_min_max_def(sample, -2147483648.0, 2147483647.0);
                t_d[i * ch + c] = (float_t)sample;
            }
    ...
    if (w->bytes == 0x01 && w->format == 0x01)
        for (size_t i = 0; i < samples; i++)
            for (size_t c = 0; c < ch; c++) {
                ...
                sample = clamp_def(sample, -1.0, 1.0);
                sample = mult_min_max_def(sample, -128.0, 127.0);
        ...
        for (size_t i = 0; i < samples; i++)
            for (size_t c = 0; c < ch; c++) {
                double_t sample = data[i * ch + c];
                sample = clamp_def(sample, -1.0, 1.0);
                sample = mult_min_max_def(sample, -32768.0, 32767.0);
        ...
        for (size_t i = 0; i < samples; i++)
            for (size_t c = 0; c < ch; c++) {
                double_t sample = data[i * ch + c];
                sample = clamp_def(sample, -1.0, 1.0);
                sample = mult_min_max_def(sample, -8388608.0, 8388607.0);
        ...
        for (size_t i = 0; i < samples; i++)
            for (size_t c = 0; c < ch; c++) {
                double_t sample = data[i * ch + c];
                sample = clamp_def(sample, -1.0, 1.0);
                sample = mult_min_max_def(sample, -2147483648.0, 2147483647.0);
