...
const char* str_utils_get_next_string(const char* str, string* value, char separator) {
    if (!str)
        return 0;
...
const wchar_t* str_utils_get_next_string(const wchar_t* str, wstring* value, wchar_t separator) {
    if (!str)
        return 0;
